  ...
  return (
    <section className="services overflow-x-hidden px-4 lg:px-0">
      <div className="container mx-auto flex flex-col items-center justify-between py-8 lg:flex-row lg:py-16">
        ...
        <div className="wrap-cards flex w-full flex-col gap-4 font-jost sm:flex-row lg:w-9/12">
          {services.map((item, index) => (
            <div
              data-aos={item.aos}
              className={`card flex max-w-full shrink-0 flex-col items-center gap-4 rounded-3xl border border-light px-4 py-4 shadow sm:w-1/3 lg:gap-8 lg:px-8 lg:py-24 ${index == 1 ? "lg:ofb-shadow" : ""}`}
              key={index}
            >
